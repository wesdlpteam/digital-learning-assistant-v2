...
- Name the specific Minecraft library lesson and include its direct URL.
- Keep Minecraft descriptions SHORT: exactly 2 practical sentences, ideally 45-70 words total.
- Do NOT paste or paraphrase the full Minecraft lesson overview, standards text, NGSS codes, or library description.
- Do NOT use vague filler such as "launchpad", "unit-connected build", or "build or investigation".
- Sentence 1 should say which verified lesson students use and the main classroom purpose.
- Sentence 2 should say the concrete student product, using simple evidence such as screenshots, signs, a short explanation, map, model, or reflection.
- If the lesson fit cannot be explained clearly in 2 sentences, do NOT propose Minecraft for that unit.
...
async function fixAllOfType(type){
  ...
  for(const idxStr of entries){
    ...
    try{
      ...
      const sugs=JSON.parse(clean.slice(si,ei+1));
      if(!sugs.length) throw new Error('Empty suggestions returned');

      // Validate no duplicates in the returned suggestions
      const toolNames = sugs.map(s=>(s.t||'').toLowerCase().trim());
      const uniqueTools = new Set(toolNames);
      if(uniqueTools.size < toolNames.length) throw new Error('AI returned duplicates again — will retry on next run');
