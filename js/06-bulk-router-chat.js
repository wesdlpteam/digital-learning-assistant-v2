...
/* ===== DLA BULK UNIVERSAL DRAFT QUALITY GUARD =====
   Applies to every review popup opened after this module loads. It cleans
   mojibake/semicolon leakage and strengthens vague safe-draft wording so every
   proposed replacement says what students do, what they create, and why it
   connects to the unit. Review-only behaviour is unchanged. */
(function(){
  if(window.__DLA_BULK_QUALITY_GUARD_INSTALLED__) return;
  window.__DLA_BULK_QUALITY_GUARD_INSTALLED__ = true;

  function qgText_(v){ return String(v == null ? '' : v); }
  function qgEscRegex_(s){ return qgText_(s).replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); }
  function qgClean_(value){
    let s = qgText_(value);
    try{
      if(typeof bulkSafeDraftFinaliseDescription_ === 'function') s = bulkSafeDraftFinaliseDescription_(s);
      else if(typeof bulkSafeDraftCleanMojibake_ === 'function') s = bulkSafeDraftCleanMojibake_(s);
    }catch(e){}
    const semicolonCount = (s.match(/;/g) || []).length;
    const wordCount = (s.match(/[A-Za-z0-9’'-]+/g) || []).length || 1;
    if(semicolonCount >= 5 && semicolonCount / wordCount > 0.10){
      s = s.replace(/;\s*/g, ' ');
    }
    s = s
      .replace(/How;\s*We;\s*Organise;\s*Ourselves/gi, 'How We Organise Ourselves')
  ...
  function qgKey_(tool){
    let t = qgText_(tool).toLowerCase().replace(/[’']/g,'').replace(/\s+/g,' ').trim();
    try{ if(typeof normaliseToolName === 'function') t = normaliseToolName(tool).toLowerCase().replace(/[’']/g,'').replace(/\s+/g,' ').trim(); }catch(e){}
    return t.replace(/[^a-z0-9]+/g,' ').trim();
  }
  function qgFocus_(entry){
    entry = entry || {};
    try{ if(typeof bulkSafeDraftUnitFocus_ === 'function') return bulkSafeDraftUnitFocus_(entry); }catch(e){}
    const theme = qgClean_(entry.th || entry.theme || 'this unit').replace(/[.!?;:]+$/,'');
    const ci = qgClean_(entry.ci || entry.centralIdea || '').replace(/[.!?;:]+$/,'');
    let loi = entry.loi || entry.lo || entry.linesOfInquiry || '';
    if(Array.isArray(loi)) loi = loi.join('; ');
    loi = qgClean_(loi).replace(/[.!?;:]+$/,'');
    if(loi) return { theme, ci, loi, connection:`the line of inquiry “${loi}”`, plannerSource:'line of inquiry' };
    if(ci) return { theme, ci, loi, connection:`the central idea “${ci}”`, plannerSource:'central idea' };
    return { theme, ci, loi, connection:`the unit theme “${theme}”`, plannerSource:'unit theme' };
  }
  function qgYearBand_(entry){
    try{ if(typeof bulkSafeDraftYearBand_ === 'function') return bulkSafeDraftYearBand_(entry || {}); }catch(e){}
    const raw = qgText_((entry && (entry.yl || entry.year)) || '').toLowerCase();
    const m = raw.match(/([0-6])/);
  ...
  function qgAgePhrase_(entry){
    const band = qgYearBand_(entry);
    if(band === 'early') return 'with teacher support';
    if(band === 'upper') return 'using evidence, design choices and a short reflection';
    return 'with clear labels and a short explanation';
  }
  function qgGenericProduct_(toolKey){
    if(/canva|green screen|remove background/.test(toolKey)) return 'short explainer video or visual campaign';
    if(/book creator/.test(toolKey)) return 'multimodal digital book';
    if(/makey/.test(toolKey)) return 'interactive touch model and Scratch quiz or soundboard';
    if(/lego spike/.test(toolKey)) return 'coded prototype that can be tested and improved';
    if(/minecraft/.test(toolKey)) return 'guided build, walkthrough and reflection';
    if(/padlet/.test(toolKey)) return 'curated evidence wall with captions and responses';
    if(/mapmaker|map maker/.test(toolKey)) return 'annotated inquiry map with labels and evidence';
    if(/google maps/.test(toolKey)) return 'Street View observation log and comparison notes';
    if(/adobe express/.test(toolKey)) return 'short visual explainer or narrated media piece';
    if(/garageband|podcast/.test(toolKey)) return 'short podcast segment with scripted narration';
    if(/scratch/.test(toolKey)) return 'interactive quiz, animation or simulation';
    if(/delightex|cospaces/.test(toolKey)) return 'interactive 3D scene with labelled hotspots';
    if(/micro:?bit/.test(toolKey)) return 'coded sensor prototype or data display';
    if(/sphero|bee bot|beebot/.test(toolKey)) return 'coded route challenge with a mapped explanation';
    if(/tinkercad|3d printer/.test(toolKey)) return 'labelled 3D model or prototype';
    if(/forms/.test(toolKey)) return 'survey, quiz or data collection form with a summary graph';
    if(/imovie|stop motion|chatterpix|puppet pals/.test(toolKey)) return 'short student-made video or animation';
    return 'digital product for a real class audience';
  }
  function qgTemplate_(tool, entry, oldDesc){
    const key = qgKey_(tool);
    const focus = qgFocus_(entry);
    const age = qgAgePhrase_(entry);
    const product = qgGenericProduct_(key);
    const theme = focus.theme || 'this unit';

    if(/green screen|remove background/.test(key)){
      return `Students use Canva’s Remove Background feature to film themselves as a reporter, storyteller or expert standing in front of a digital backdrop connected to ${focus.connection}. They create a short explainer video, virtual tour or news report that shows a specific place, scene, artefact or idea from ${theme}, then explain how the background and script help communicate their understanding.`;
    }
    if(/canva/.test(key)){
      return `Students use Canva to create a ${product} connected to ${focus.connection}. They combine images, headings, short text and voice or video elements to explain a clear idea from ${theme}, then publish it for classmates or families as an audience-facing learning product.`;
    }
    if(/book creator/.test(key)){
      return `Students use Book Creator to publish a ${product} connected to ${focus.connection}. Each page combines drawings, photos, captions and optional voice recordings so students can explain examples, vocabulary and reflections from ${theme} ${age}.`;
    }
    if(/makey/.test(key)){
      return `Students use Makey Makey to build an ${product} connected to ${focus.connection}. Classmates press parts of the model to trigger sounds, questions or feedback, then students explain how the interaction reveals key ideas from ${theme}.`;
    }
    if(/lego spike/.test(key)){
      return `Students use Lego Spike Prime to design, build, code and test a ${product} connected to ${focus.connection}. They record what changed after testing and explain how the prototype helps model, solve or communicate an idea from ${theme}.`;
    }
    if(/minecraft/.test(key)){
      return `Students use the curated Minecraft lesson to create a ${product} connected to ${focus.connection}. They complete the build or challenge, capture screenshots or a walkthrough, and explain how their design decisions show evidence of understanding from ${theme}.`;
    }
    if(/padlet/.test(key)){
      return `Students use Padlet to build a ${product} connected to ${focus.connection}. Each post includes an image, short explanation and question or response so the class can compare evidence and build a shared understanding of ${theme}.`;
    }
    if(/mapmaker|map maker/.test(key)){
      return `Students use National Geographic MapMaker to create a ${product} connected to ${focus.connection}. They add locations, labels, symbols and short evidence notes, then explain the spatial patterns or relationships they notice in ${theme}.`;
    }
    if(/google maps/.test(key)){
      return `Students use Google Maps Street View for ground-level virtual observation connected to ${focus.connection}. They compare real places, record what they notice in an observation log, and explain how the location evidence helps them understand ${theme}.`;
    }
    if(/adobe express podcasting|garageband|podcast/.test(key)){
      return `Students create a ${product} connected to ${focus.connection}. They script roles such as host, expert and interviewee, record clear evidence-based explanations, and publish a short audio piece that helps an audience understand ${theme}.`;
    }
    if(/adobe express/.test(key)){
      return `Students use Adobe Express to create a ${product} connected to ${focus.connection}. They combine visuals, concise text and narration to explain a claim or perspective from ${theme}, then share it with a defined audience.`;
    }
    if(/scratch/.test(key)){
      return `Students use Scratch to create an ${product} connected to ${focus.connection}. Users click, answer or move through the project to reveal key ideas, and students explain how their code choices help communicate ${theme}.`;
    }
    if(/delightex|cospaces/.test(key)){
      return `Students use Delightex to create an ${product} connected to ${focus.connection}. They build a scene with objects, labels and interactive hotspots, then guide classmates through the space to explain their understanding of ${theme}.`;
    }
    if(/micro:?bit/.test(key)){
      return `Students use Micro:bit to code a ${product} connected to ${focus.connection}. They test the input or sensor response, collect observations, and explain how the coded output models or measures an idea from ${theme}.`;
    }
    if(/sphero|bee bot|beebot/.test(key)){
      return `Students code a ${product} connected to ${focus.connection}. They plan, test and debug the route, then annotate the path to explain how each movement represents an idea or relationship from ${theme}.`;
    }
    if(/tinkercad|3d printer/.test(key)){
      return `Students design a ${product} connected to ${focus.connection}. They label important features, explain their design choices, and reflect on how the prototype helps communicate an idea from ${theme}.`;
    }
    if(/forms/.test(key)){
      return `Students use Microsoft Forms to create a ${product} connected to ${focus.connection}. They collect class responses, analyse the results in a simple graph or summary, and explain what the evidence suggests about ${theme}.`;
    }
    if(/imovie|stop motion|chatterpix|puppet pals/.test(key)){
      return `Students create a ${product} connected to ${focus.connection}. They plan scenes, record narration or dialogue, and use the finished media to explain a process, perspective or story from ${theme}.`;
    }
    return `Students use ${tool || 'the selected tool'} to create a ${product} connected to ${focus.connection}. They make choices, add evidence or explanation, and share the finished product with classmates to show what they understand about ${theme}.`;
  }
  function qgEnhanceChange_(change){
    if(!change || typeof change !== 'object') return change;
    const out = Object.assign({}, change);
    const entry = (typeof DATA !== 'undefined' && DATA && Number.isInteger(out.entryIdx)) ? DATA[out.entryIdx] : null;
    ...
    // Guard against physical green screen kit wording. Wesley uses Canva's Remove Background pathway.
    if(/green\s*screen/i.test(tool) || /green\s*screen/i.test(originalDesc)){
      out.t = 'Canva';
      desc = qgTemplate_('Canva Remove Background', entry, originalDesc);
    } else if(out.t != null) out.t = tool;
    else if(out.tool != null) out.tool = tool;
    else if(out.technology != null) out.technology = tool;
    else if(out.name != null) out.name = tool;

    if(out.d != null || (!out.desc && !out.description && !out.integration_idea && !out.activity && !out.suggestion)) out.d = desc;
    else if(out.desc != null) out.desc = desc;
    else if(out.description != null) out.description = desc;
    else if(out.integration_idea != null) out.integration_idea = desc;
  ...
  function qgInstall_(){
    if(typeof window.showChangesPopup !== 'function') return false;
    if(window.showChangesPopup.__dlaQualityGuardWrapped) return true;
    ...
    wrapped.__dlaQualityGuardWrapped = true;
    window.showChangesPopup = wrapped;
    return true;
  }
  if(!qgInstall_()){
    let tries = 0;
    const timer = setInterval(function(){
      tries++;
      if(qgInstall_() || tries > 80) clearInterval(timer);
    }, 250);
  }
})();
