...
function bulkDetectRemovalTool_(instruction){
  ...
  const aliases = {
    ...
    'adobe podcast':'Adobe Express Podcasting',
    'podcasting':'Adobe Express Podcasting',
    'microbit':'Micro:bit',
    'micro bit':'Micro:bit',
    'micro:bit':'Micro:bit',
...
// ========== NAMED TOOL OPPORTUNITY FLOW ==========
// Handles prompts such as “find more opportunities to use Makey Makey”.
// This is intentionally local/deterministic: it finds targets locally and drafts
// review-only changes without one AI call per candidate.
function bulkInstructionTargetsNamedToolOpportunity_(instruction){
  ...
  // Keep these specialised tools on their existing flows.
...
function bulkToolOpportunityFamily_(toolName){
  ...
  if(/google earth|google maps|field guide|mapmaker/.test(key)) return 'place';
...
function bulkToolOpportunityUnitScore_(entry, toolName, instruction){
  ...
  if(family === 'stem-hardware'){
    ...
    if(/how we express ourselves|art|story|culture|celebration|performance|music/.test(unit) && !/interactive|mechanism|prototype|design|model/.test(unit)) score -= 4;
  ...
  if(/makey makey/i.test(toolName)){
    if(/circuit|electric|conduct|sound|interactive|prototype|model|music|design|cause and effect|sensor|input|output/.test(unit)) score += 10;
  }
...
async function runBulkNamedToolOpportunityFlow_(instruction, completeData, prog, lbl, bar){
  ...
  const defaultCap = Math.min(8, allCandidates.length); // deliberately modest first run
  const cap = explicitCount ? Math.min(explicitCount, allCandidates.length, 20) : defaultCap;
  ...
  if(!selected.length){
    ...
    const yearText = targetYears.length ? ` in ${targetYears.join(', ')}` : '';
    const parts = [];
    if(skippedYear) parts.push(`${skippedYear} outside the requested year level`);
  ...
  updateReasoningStep(2, 'done');
  updateReasoningStep(3, 'done');
  updateReasoningStep(4, 'active', `Preparing ${selected.length} ${targetTool} review drafts`);
  if(lbl) lbl.textContent = `Preparing ${selected.length} ${targetTool} review drafts…`;
...
async function startBulkAnalysis(){
  ...
  // Specific classroom-tool opportunity searches should be handled locally first.
  // Example: “find more opportunities to use Makey Makey”. The generic GPT scan
  // can accidentally rewrite the one existing Makey Makey slot instead of finding new units.
  if(bulkInstructionTargetsNamedToolOpportunity_(enrichedInstruction)){
    await runBulkNamedToolOpportunityFlow_(enrichedInstruction, completeData, prog, lbl, bar);
    return;
  }
