...
- Frontend POSTs to `SCRIPT_URL` (in `00-config-state-utils.js`) use `mode:'no-cors'`, so **responses cannot be read**. The Studio assumes success and polls Drive afterwards. When adding new actions, do not rely on response bodies on the client.
