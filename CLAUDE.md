...
Single `doPost(e)` router dispatched by `body.action` (lowercased). Current actions: `runSurgeon`, `addToQueue`, `callAI`, `getPlannerContext`, `syncToolInventory`, `enrichPlanners`, `rebootMakerspace`, `resetMakerspaceFlags`, `extractUnitDetails`, `extraTechIdeas`. Every action goes through `requireAllowedUser_` first.
...
### Sandbox demo (`tools/`, output in `../dla-sandbox`)
`node tools/build-sandbox.mjs --out ../dla-sandbox` regenerates the presentation copy of the site and Studio. Never hand-edit anything in `dla-sandbox` — the build overwrites it. `npm test` covers the transforms, the guard, and the demo's tool picker.

The demo makes **no AI calls at runtime**, so anything the picker shows must be baked in at build time:
- Each unit's own six ideas come from `data.json`.
- The "Have a tech tool in mind?" picker falls back to `sandbox-extra-ideas.json` when a tapped tool has no ready-made idea for the unit. That file is generated by `node tools/gen-sandbox-extra-ideas.mjs` (auth reuses the `clasp` login; calls the backend's `extraTechIdeas`), is committed, resumable per unit, and only needs re-running when the unit corpus changes. It is fetched lazily by the shim, so it is not part of the first-load size budget.
...
- **Studio cache versioning — bump on EVERY Studio change.** Browsers cache the Studio's files hard. Any change to `DLA_Studio.html`, `js/*.js`, or `css/studio.css` must bump `APP_VERSION` in `js/00-config-state-utils.js` AND the matching `?v=` query strings on all script/css tags at the bottom of `DLA_Studio.html`, together, or users keep running stale code. The Studio shows the running version bottom-left; the ↻ badge force-reloads to latest.
- **Public site auto-refresh needs its own bump.** `index.html` auto-refreshes open teacher tabs when it detects a new deploy — but only if `window.APP_VERSION` (line 4 of `index.html`) is bumped with each deploy. Forget the bump and the mechanism silently does nothing.
- **AI prompt rules exist in TWO copies and drift.** Suggestion-style/eligibility rules live in both the Studio client (`js/05-…`, `js/06-…`) and the server (`gas_backend/Code.js`). Whole-unit regeneration runs SERVER-side, so a client-only rule edit does nothing there. Patch both copies, then ship both (git push for the Studio, `clasp push` + deployment bump for the backend).
- **`gas_backend/Code.js` line endings are CRLF — edit with care.** Don't edit it with tools that normalise line endings. Write changes via a script that preserves CRLF (e.g. Python `open(path, 'w', newline='')` with CRLF strings), then a plain `git add` (autocrlf cleans on commit). Never `git -c core.autocrlf=false add` — it poisons the index and the next diff becomes a whole-file change.
