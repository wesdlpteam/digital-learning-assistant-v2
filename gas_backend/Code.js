...
const OPENAI_FAST_MODEL = 'gpt-5.6-terra';
...
- Tinkercad truth: Tinkercad is for designing and modelling 3D objects to 3D-print in plastic. ONLY suggest it for designing a 3D object or prototype to print. Do NOT frame it as electronics, circuits, breadboards, simulation or block-coding -- Tinkercad Circuits and Codeblocks are OFF-LIMITS at Wesley. Do NOT use it to simulate, test or compare material properties (strength, flexibility, weight, recyclability, or plastic vs metal vs wood) -- it only designs shapes in plastic and cannot simulate materials. Mirrors the Studio-side Tinkercad rule in js/05 TOOL_AFFORDANCE_NOTES; keep the two in sync.
...
function doPost(e) {
  try {
    ...
    if (action === 'repairscrambledtext') {
      ...
      return jsonResponse(repairResult);
    }
...
// ==========================================
// 2026-07-07: One-time corpus text repair — literal '?' left by lossy
// transcodes of ligatures (di?erent), apostrophes (didn?t), em-dashes
// (clip?complete) and NBSPs in planner soup. Exact-token maps built from a
// reviewed corpus inventory (docs/superpowers/plans/2026-07-07-system-audit-fixes.md).
// 'all' applies to every string field; 'dash' only to teacher-visible
// suggestion text (d/from/fromTool → em-dash); 'space' to internal planner
// fields (NBSP loss → space). URL query tokens (watch?v etc.) are excluded
// from the maps, so links are untouchable by construction.
// ==========================================
var REPAIR_TEXT_MAP = {"all":[{"from":"makerspace?cardboard","to":"makerspace: cardboard"},{"from":"reimagined?physical","to":"reimagined: physical"},{"from":"biodiversity?s","to":"biodiversity's"},{"from":"doppelg?ngers","to":"doppelgängers"},{"from":"papier-m?ch?","to":"papier-mâché"},{"from":"character?s","to":"character's"},{"from":"australia?s","to":"australia's"},{"from":"animat?ing","to":"animating"},{"from":"animatin?g","to":"animating"},{"from":"platform?s","to":"platform's"},{"from":"animati?ng","to":"animating"},{"from":"organism?s","to":"organism's"},{"from":"everyone?s","to":"everyone's"},{"from":"product?s","to":"product's"},{"from":"creator?s","to":"creator's"},{"from":"couldn?t","to":"couldn't"},{"from":"street?s","to":"street's"},{"from":"wouldn?t","to":"wouldn't"},{"from":"padlet?s","to":"padlet's"},{"from":"family?s","to":"family's"},{"from":"puppet?s","to":"puppet's"},{"from":"people?s","to":"people's"},{"from":"doesn?t","to":"doesn't"},{"from":"weren?t","to":"weren't"},{"from":"haven?t","to":"haven't"},{"from":"pok?mon","to":"pokémon"},{"from":"badge?s","to":"badge's"},{"from":"peter?s","to":"peter's"},{"from":"canva?s","to":"canva's"},{"from":"makey?s","to":"makey's"},{"from":"didn?t","to":"didn't"},{"from":"wasn?t","to":"wasn't"},{"from":"hadn?t","to":"hadn't"},{"from":"bene?t","to":"benefit"},{"from":"t?hoku","to":"tōhoku"},{"from":"aren?t","to":"aren't"},{"from":"hasn?t","to":"hasn't"},{"from":"indi?s","to":"indi's"},{"from":"unit?s","to":"unit's"},{"from":"epic?s","to":"epic's"},{"from":"cube?s","to":"cube's"},{"from":"don?t","to":"don't"},{"from":"pro?t","to":"profit"},{"from":"isn?t","to":"isn't"},{"from":"can?t","to":"can't"},{"from":"app?s","to":"app's"},{"from":"won?t","to":"won't"},{"from":"edu?s","to":"edu's"}],"dash":[{"from":"soundscapes?thunderstorms","to":"soundscapes—thunderstorms"},{"from":"future-shapers?entrusted","to":"future-shapers—entrusted"},{"from":"communicator?empowering","to":"communicator—empowering"},{"from":"commentary?highlighting","to":"commentary—highlighting"},{"from":"indigenous?highlighting","to":"indigenous—highlighting"},{"from":"perspectives?including","to":"perspectives—including"},{"from":"landmarks?representing","to":"landmarks—representing"},{"from":"experience?respondents","to":"experience—respondents"},{"from":"recommendations?shared","to":"recommendations—shared"},{"from":"campaign?infographics","to":"campaign—infographics"},{"from":"communicate?practical","to":"communicate—practical"},{"from":"disaster?spotlighting","to":"disaster—spotlighting"},{"from":"exploration?including","to":"exploration—including"},{"from":"imagined?highlighting","to":"imagined—highlighting"},{"from":"connections?sometimes","to":"connections—sometimes"},{"from":"message?demonstrated","to":"message—demonstrated"},{"from":"contributors?parents","to":"contributors—parents"},{"from":"travellers?confident","to":"travellers—confident"},{"from":"changemakers?sharing","to":"changemakers—sharing"},{"from":"communities?inviting","to":"communities—inviting"},{"from":"environments?mosques","to":"environments—mosques"},{"from":"resources?navigating","to":"resources—navigating"},{"from":"protection?mirroring","to":"protection—mirroring"},{"from":"advertising?aligning","to":"advertising—aligning"},{"from":"scoreboards?teaching","to":"scoreboards—teaching"},{"from":"perspective?european","to":"perspective—european"},{"from":"perspectives?marking","to":"perspectives—marking"},{"from":"background?triggered","to":"background—triggered"},{"from":"interactive?letting","to":"interactive—letting"},{"from":"eco-challenges?like","to":"eco-challenges—like"},{"from":"classmates?swapping","to":"classmates—swapping"},{"from":"communication?think","to":"communication—think"},{"from":"custodians?curating","to":"custodians—curating"},{"from":"perspective?turning","to":"perspective—turning"},{"from":"class?experimenting","to":"class—experimenting"},{"from":"storyboard?complete","to":"storyboard—complete"},{"from":"perspectives?miners","to":"perspectives—miners"},{"from":"journal?screenshots","to":"journal—screenshots"},{"from":"milestones?blending","to":"milestones—blending"},{"from":"problem-solvers?not","to":"problem-solvers—not"},{"from":"influences?suddenly","to":"influences—suddenly"},{"from":"character?sometimes","to":"character—sometimes"},{"from":"empowerment?knowing","to":"empowerment—knowing"},{"from":"interviews?students","to":"interviews—students"},{"from":"illustrations?ready","to":"illustrations—ready"},{"from":"soundboard?pressing","to":"soundboard—pressing"},{"from":"perspective?letting","to":"perspective—letting"},{"from":"process?predicting","to":"process—predicting"},{"from":"categories?natural","to":"categories—natural"},{"from":"capsule?collecting","to":"capsule—collecting"},{"from":"scenarios?inviting","to":"scenarios—inviting"},{"from":"fix?sustainability","to":"fix—sustainability"},{"from":"breakthroughs?have","to":"breakthroughs—have"},{"from":"inspiration?shared","to":"inspiration—shared"},{"from":"communication?each","to":"communication—each"},{"from":"expression?imagine","to":"expression—imagine"},{"from":"segments?imagining","to":"segments—imagining"},{"from":"prototype?complete","to":"prototype—complete"},{"from":"campaigns?breaking","to":"campaigns—breaking"},{"from":"storytelling?ready","to":"storytelling—ready"},{"from":"dreams?represented","to":"dreams—represented"},{"from":"hard?investigating","to":"hard—investigating"},{"from":"sculptures?tapping","to":"sculptures—tapping"},{"from":"animation?complete","to":"animation—complete"},{"from":"patterns?revealing","to":"patterns—revealing"},{"from":"materials?students","to":"materials—students"},{"from":"landscape?inviting","to":"landscape—inviting"},{"from":"collection?student","to":"collection—student"},{"from":"opportunities?just","to":"opportunities—just"},{"from":"discoveries?adding","to":"discoveries—adding"},{"from":"materials?complete","to":"materials—complete"},{"from":"empathy?reflecting","to":"empathy—reflecting"},{"from":"dashboard?complete","to":"dashboard—complete"},{"from":"media?interconnect","to":"media—interconnect"},{"from":"reflections?shared","to":"reflections—shared"},{"from":"trees?highlighting","to":"trees—highlighting"},{"from":"artefact?imagining","to":"artefact—imagining"},{"from":"challenges?sharing","to":"challenges—sharing"},{"from":"communities?should","to":"communities—should"},{"from":"production?pushing","to":"production—pushing"},{"from":"artefacts?artworks","to":"artefacts—artworks"},{"from":"classroom?sparking","to":"classroom—sparking"},{"from":"music?demonstrated","to":"music—demonstrated"},{"from":"encouragement?kids","to":"encouragement—kids"},{"from":"characters?making","to":"characters—making"},{"from":"masterpieces?each","to":"masterpieces—each"},{"from":"child?discovering","to":"child—discovering"},{"from":"environments?such","to":"environments—such"},{"from":"significance?then","to":"significance—then"},{"from":"carers?responding","to":"carers—responding"},{"from":"walk-through?that","to":"walk-through—that"},{"from":"migrants?sparking","to":"migrants—sparking"},{"from":"colonisation?past","to":"colonisation—past"},{"from":"materials?imagine","to":"materials—imagine"},{"from":"frustration?using","to":"frustration—using"},{"from":"expression?visual","to":"expression—visual"},{"from":"judges?architects","to":"judges—architects"},{"from":"feedback?flipping","to":"feedback—flipping"},{"from":"entries?published","to":"entries—published"},{"from":"outcomes?friendly","to":"outcomes—friendly"},{"from":"campaign?animated","to":"campaign—animated"},{"from":"features?liftable","to":"features—liftable"},{"from":"questioners?ready","to":"questioners—ready"},{"from":"boxes?accompanied","to":"boxes—accompanied"},{"from":"interactions?both","to":"interactions—both"},{"from":"laughter?children","to":"laughter—children"},{"from":"episode?complete","to":"episode—complete"},{"from":"discoveries?like","to":"discoveries—like"},{"from":"child?regardless","to":"child—regardless"},{"from":"repeats?students","to":"repeats—students"},{"from":"narration?shared","to":"narration—shared"},{"from":"spaces?sometimes","to":"spaces—sometimes"},{"from":"exploration?like","to":"exploration—like"},{"from":"questions?shared","to":"questions—shared"},{"from":"species?bringing","to":"species—bringing"},{"from":"voice?especially","to":"voice—especially"},{"from":"moments?bridging","to":"moments—bridging"},{"from":"effects?learning","to":"effects—learning"},{"from":"groups?democracy","to":"groups—democracy"},{"from":"electricity?used","to":"electricity—used"},{"from":"accessories?such","to":"accessories—such"},{"from":"stories?blending","to":"stories—blending"},{"from":"ingredients?such","to":"ingredients—such"},{"from":"story?aboriginal","to":"story—aboriginal"},{"from":"projects?whether","to":"projects—whether"},{"from":"artwork?sparking","to":"artwork—sparking"},{"from":"reflections?that","to":"reflections—that"},{"from":"modelling?shared","to":"modelling—shared"},{"from":"disturbance?that","to":"disturbance—that"},{"from":"storytelling?and","to":"storytelling—and"},{"from":"branch?imagining","to":"branch—imagining"},{"from":"buzzer?mirroring","to":"buzzer—mirroring"},{"from":"story?indigenous","to":"story—indigenous"},{"from":"checkpoints?such","to":"checkpoints—such"},{"from":"explanation?such","to":"explanation—such"},{"from":"fruits?mirroring","to":"fruits—mirroring"},{"from":"games?excitement","to":"games—excitement"},{"from":"celebration?like","to":"celebration—like"},{"from":"journal?complete","to":"journal—complete"},{"from":"newsletter?ready","to":"newsletter—ready"},{"from":"strategies?using","to":"strategies—using"},{"from":"action?sometimes","to":"action—sometimes"},{"from":"students?meaning","to":"students—meaning"},{"from":"audience?posting","to":"audience—posting"},{"from":"journey?complete","to":"journey—complete"},{"from":"factors?opinions","to":"factors—opinions"},{"from":"interviews?ready","to":"interviews—ready"},{"from":"neighbours?using","to":"neighbours—using"},{"from":"materials?leaves","to":"materials—leaves"},{"from":"challenges?like","to":"challenges—like"},{"from":"feedback?lights","to":"feedback—lights"},{"from":"resources?right","to":"resources—right"},{"from":"board?organised","to":"board—organised"},{"from":"creature?laying","to":"creature—laying"},{"from":"rovers?handmade","to":"rovers—handmade"},{"from":"conflicts?using","to":"conflicts—using"},{"from":"spaces?bridging","to":"spaces—bridging"},{"from":"government?from","to":"government—from"},{"from":"minds?including","to":"minds—including"},{"from":"sequence?photos","to":"sequence—photos"},{"from":"text?showcasing","to":"text—showcasing"},{"from":"posts?fostering","to":"posts—fostering"},{"from":"textures?adding","to":"textures—adding"},{"from":"changes?bending","to":"changes—bending"},{"from":"crushing?before","to":"crushing—before"},{"from":"soundscape?that","to":"soundscape—that"},{"from":"behaviours?such","to":"behaviours—such"},{"from":"audience?little","to":"audience—little"},{"from":"scorekeeping?to","to":"scorekeeping—to"},{"from":"automata?moving","to":"automata—moving"},{"from":"report?complete","to":"report—complete"},{"from":"journals?mixing","to":"journals—mixing"},{"from":"prototype?think","to":"prototype—think"},{"from":"patterns?making","to":"patterns—making"},{"from":"outcome?perfect","to":"outcome—perfect"},{"from":"items?triggered","to":"items—triggered"},{"from":"navigators?able","to":"navigators—able"},{"from":"recording?ready","to":"recording—ready"},{"from":"mindfulness?and","to":"mindfulness—and"},{"from":"feeling?turning","to":"feeling—turning"},{"from":"conditions?like","to":"conditions—like"},{"from":"adaptation?like","to":"adaptation—like"},{"from":"tradition?using","to":"tradition—using"},{"from":"materials?think","to":"materials—think"},{"from":"differences?and","to":"differences—and"},{"from":"notes?sometimes","to":"notes—sometimes"},{"from":"wellbeing?alive","to":"wellbeing—alive"},{"from":"lifestyle?think","to":"lifestyle—think"},{"from":"mechanisms?like","to":"mechanisms—like"},{"from":"most?creativity","to":"most—creativity"},{"from":"change?students","to":"change—students"},{"from":"campaigns?think","to":"campaigns—think"},{"from":"knowledge?shape","to":"knowledge—shape"},{"from":"elements?colour","to":"elements—colour"},{"from":"shop?decorating","to":"shop—decorating"},{"from":"materials?like","to":"materials—like"},{"from":"scenarios?such","to":"scenarios—such"},{"from":"character?such","to":"character—such"},{"from":"resources?such","to":"resources—such"},{"from":"character?like","to":"character—like"},{"from":"scenarios?like","to":"scenarios—like"},{"from":"styles?helping","to":"styles—helping"},{"from":"homes?choosing","to":"homes—choosing"},{"from":"solutions?like","to":"solutions—like"},{"from":"teamwork?using","to":"teamwork—using"},{"from":"video?complete","to":"video—complete"},{"from":"solution?ready","to":"solution—ready"},{"from":"variables?like","to":"variables—like"},{"from":"materials?have","to":"materials—have"},{"from":"artefacts?such","to":"artefacts—such"},{"from":"classmates?and","to":"classmates—and"},{"from":"guide?complete","to":"guide—complete"},{"from":"album?complete","to":"album—complete"},{"from":"cardboard?that","to":"cardboard—that"},{"from":"management?are","to":"management—are"},{"from":"sanitation?are","to":"sanitation—are"},{"from":"disasters?such","to":"disasters—such"},{"from":"members?elders","to":"members—elders"},{"from":"listening?that","to":"listening—that"},{"from":"choices?should","to":"choices—should"},{"from":"diary?complete","to":"diary—complete"},{"from":"events?perfect","to":"events—perfect"},{"from":"imagined?using","to":"imagined—using"},{"from":"stage?planting","to":"stage—planting"},{"from":"itself?sharing","to":"itself—sharing"},{"from":"materials?used","to":"materials—used"},{"from":"materials?help","to":"materials—help"},{"from":"tools?students","to":"tools—students"},{"from":"others?imagine","to":"others—imagine"},{"from":"materials?then","to":"materials—then"},{"from":"knowledge?such","to":"knowledge—such"},{"from":"highlight?such","to":"highlight—such"},{"from":"messages?ready","to":"messages—ready"},{"from":"audiences?like","to":"audiences—like"},{"from":"reading?making","to":"reading—making"},{"from":"members?mayors","to":"members—mayors"},{"from":"audience?maybe","to":"audience—maybe"},{"from":"explorers?from","to":"explorers—from"},{"from":"questions?then","to":"questions—then"},{"from":"difference?and","to":"difference—and"},{"from":"entry?complete","to":"entry—complete"},{"from":"change?whether","to":"change—whether"},{"from":"event?complete","to":"event—complete"},{"from":"step?resources","to":"step—resources"},{"from":"discovery?such","to":"discovery—such"},{"from":"lives?clothing","to":"lives—clothing"},{"from":"alone?sparking","to":"alone—sparking"},{"from":"idea?artefacts","to":"idea—artefacts"},{"from":"system?perfect","to":"system—perfect"},{"from":"energy?imagine","to":"energy—imagine"},{"from":"playdough?into","to":"playdough—into"},{"from":"cycles?letting","to":"cycles—letting"},{"from":"journeys?think","to":"journeys—think"},{"from":"experience?far","to":"experience—far"},{"from":"remembered?and","to":"remembered—and"},{"from":"scene?complete","to":"scene—complete"},{"from":"answers?making","to":"answers—making"},{"from":"decisions?from","to":"decisions—from"},{"from":"stories?behind","to":"stories—behind"},{"from":"challenging?of","to":"challenging—of"},{"from":"narrative?real","to":"narrative—real"},{"from":"theme?conflict","to":"theme—conflict"},{"from":"boundaries?and","to":"boundaries—and"},{"from":"scene?learning","to":"scene—learning"},{"from":"effects?chirps","to":"effects—chirps"},{"from":"touched?giving","to":"touched—giving"},{"from":"layers?drawing","to":"layers—drawing"},{"from":"elements?like","to":"elements—like"},{"from":"patterns?like","to":"patterns—like"},{"from":"messages?such","to":"messages—such"},{"from":"characters?no","to":"characters—no"},{"from":"routines?such","to":"routines—such"},{"from":"emotions?like","to":"emotions—like"},{"from":"features?like","to":"features—like"},{"from":"audience?year","to":"audience—year"},{"from":"clip?complete","to":"clip—complete"},{"from":"scenario?like","to":"scenario—like"},{"from":"layer?showing","to":"layer—showing"},{"from":"movement?such","to":"movement—such"},{"from":"school?litter","to":"school—litter"},{"from":"resolve?ready","to":"resolve—ready"},{"from":"friends?happy","to":"friends—happy"},{"from":"region?shared","to":"region—shared"},{"from":"library?while","to":"library—while"},{"from":"audience?like","to":"audience—like"},{"from":"view?tracking","to":"view—tracking"},{"from":"clapsticks?to","to":"clapsticks—to"},{"from":"video?viewers","to":"video—viewers"},{"from":"routines?that","to":"routines—that"},{"from":"strength?like","to":"strength—like"},{"from":"product?think","to":"product—think"},{"from":"gadgets?using","to":"gadgets—using"},{"from":"affected?then","to":"affected—then"},{"from":"conflict?such","to":"conflict—such"},{"from":"elements?such","to":"elements—such"},{"from":"democracy?and","to":"democracy—and"},{"from":"book?complete","to":"book—complete"},{"from":"identity?such","to":"identity—such"},{"from":"life?lighting","to":"life—lighting"},{"from":"template?such","to":"template—such"},{"from":"equipment?and","to":"equipment—and"},{"from":"shows?turning","to":"shows—turning"},{"from":"penguin?using","to":"penguin—using"},{"from":"school?matter","to":"school—matter"},{"from":"material?such","to":"material—such"},{"from":"material?from","to":"material—from"},{"from":"planet?making","to":"planet—making"},{"from":"changes?maybe","to":"changes—maybe"},{"from":"display?ready","to":"display—ready"},{"from":"features?such","to":"features—such"},{"from":"detection?let","to":"detection—let"},{"from":"others?skills","to":"others—skills"},{"from":"feedback?that","to":"feedback—that"},{"from":"products?from","to":"products—from"},{"from":"stories?maybe","to":"stories—maybe"},{"from":"teamwork?like","to":"teamwork—like"},{"from":"bubbles?ready","to":"bubbles—ready"},{"from":"products?like","to":"products—like"},{"from":"jeans?travels","to":"jeans—travels"},{"from":"feelings?into","to":"feelings—into"},{"from":"type?blending","to":"type—blending"},{"from":"listening?and","to":"listening—and"},{"from":"whom?sparking","to":"whom—sparking"},{"from":"life?students","to":"life—students"},{"from":"stations?each","to":"stations—each"},{"from":"lighting?that","to":"lighting—that"},{"from":"problems?what","to":"problems—what"},{"from":"collapsed?and","to":"collapsed—and"},{"from":"bridges?using","to":"bridges—using"},{"from":"dilemmas?like","to":"dilemmas—like"},{"from":"rhythm?change","to":"rhythm—change"},{"from":"symbolism?can","to":"symbolism—can"},{"from":"decorative?it","to":"decorative—it"},{"from":"missed?before","to":"missed—before"},{"from":"questions?and","to":"questions—and"},{"from":"migrant?child","to":"migrant—child"},{"from":"material?like","to":"material—like"},{"from":"fabric?change","to":"fabric—change"},{"from":"stickers?like","to":"stickers—like"},{"from":"art?displayed","to":"art—displayed"},{"from":"objects?like","to":"objects—like"},{"from":"systems?like","to":"systems—like"},{"from":"product?such","to":"product—such"},{"from":"message?such","to":"message—such"},{"from":"emotion?like","to":"emotion—like"},{"from":"history?they","to":"history—they"},{"from":"systems?such","to":"systems—such"},{"from":"routines?and","to":"routines—and"},{"from":"sources?such","to":"sources—such"},{"from":"changes?like","to":"changes—like"},{"from":"product?like","to":"product—like"},{"from":"t-shirt?from","to":"t-shirt—from"},{"from":"members?like","to":"members—like"},{"from":"moments?like","to":"moments—like"},{"from":"wildly?using","to":"wildly—using"},{"from":"sound?giving","to":"sound—giving"},{"from":"service?like","to":"service—like"},{"from":"cycles?think","to":"cycles—think"},{"from":"flower?using","to":"flower—using"},{"from":"feelings?and","to":"feelings—and"},{"from":"story?making","to":"story—making"},{"from":"readers?they","to":"readers—they"},{"from":"quizzes?that","to":"quizzes—that"},{"from":"concepts?far","to":"concepts—far"},{"from":"devices?such","to":"devices—such"},{"from":"wheels?using","to":"wheels—using"},{"from":"evoke?shared","to":"evoke—shared"},{"from":"bushfires?by","to":"bushfires—by"},{"from":"outsmart?the","to":"outsmart—the"},{"from":"decisions?an","to":"decisions—an"},{"from":"tracks?short","to":"tracks—short"},{"from":"beyond?using","to":"beyond—using"},{"from":"studies?from","to":"studies—from"},{"from":"overlooked?a","to":"overlooked—a"},{"from":"journey?from","to":"journey—from"},{"from":"routine?like","to":"routine—like"},{"from":"ideas?giving","to":"ideas—giving"},{"from":"tracks?ready","to":"tracks—ready"},{"from":"choices?that","to":"choices—that"},{"from":"bubbles?that","to":"bubbles—that"},{"from":"side?walking","to":"side—walking"},{"from":"helper?using","to":"helper—using"},{"from":"world?street","to":"world—street"},{"from":"concept?like","to":"concept—like"},{"from":"sounds?based","to":"sounds—based"},{"from":"results?like","to":"results—like"},{"from":"life?turning","to":"life—turning"},{"from":"visually?and","to":"visually—and"},{"from":"facts?shared","to":"facts—shared"},{"from":"pebbles?that","to":"pebbles—that"},{"from":"signals?that","to":"signals—that"},{"from":"students?who","to":"students—who"},{"from":"problems?and","to":"problems—and"},{"from":"sensors?like","to":"sensors—like"},{"from":"sneaker?from","to":"sneaker—from"},{"from":"choices?like","to":"choices—like"},{"from":"impact?using","to":"impact—using"},{"from":"children?and","to":"children—and"},{"from":"word?giggles","to":"word—giggles"},{"from":"stages?roots","to":"stages—roots"},{"from":"t-shirt?into","to":"t-shirt—into"},{"from":"stories?like","to":"stories—like"},{"from":"artefacts?by","to":"artefacts—by"},{"from":"values?alive","to":"values—alive"},{"from":"species?like","to":"species—like"},{"from":"poster?touch","to":"poster—touch"},{"from":"flags?tasked","to":"flags—tasked"},{"from":"journey?each","to":"journey—each"},{"from":"evidence?and","to":"evidence—and"},{"from":"platform?and","to":"platform—and"},{"from":"objects?that","to":"objects—that"},{"from":"badges?using","to":"badges—using"},{"from":"stage?farmer","to":"stage—farmer"},{"from":"identity?and","to":"identity—and"},{"from":"effects?like","to":"effects—like"},{"from":"facts?hosted","to":"facts—hosted"},{"from":"journey?then","to":"journey—then"},{"from":"ebook?filled","to":"ebook—filled"},{"from":"effects?both","to":"effects—both"},{"from":"writers?they","to":"writers—they"},{"from":"stories?such","to":"stories—such"},{"from":"ideas?matter","to":"ideas—matter"},{"from":"collage?like","to":"collage—like"},{"from":"actions?like","to":"actions—like"},{"from":"imagined?can","to":"imagined—can"},{"from":"history?and","to":"history—and"},{"from":"message?and","to":"message—and"},{"from":"gardener?by","to":"gardener—by"},{"from":"expert?like","to":"expert—like"},{"from":"holiday?and","to":"holiday—and"},{"from":"memory?such","to":"memory—such"},{"from":"feeling?joy","to":"feeling—joy"},{"from":"canvas?full","to":"canvas—full"},{"from":"images?that","to":"images—that"},{"from":"leaves?into","to":"leaves—into"},{"from":"labels?that","to":"labels—that"},{"from":"points?like","to":"points—like"},{"from":"worried?and","to":"worried—and"},{"from":"idea?forces","to":"idea—forces"},{"from":"bouncing?by","to":"bouncing—by"},{"from":"science?can","to":"science—can"},{"from":"blocks?like","to":"blocks—like"},{"from":"groups?like","to":"groups—like"},{"from":"sounds?like","to":"sounds—like"},{"from":"online?each","to":"online—each"},{"from":"language?by","to":"language—by"},{"from":"explain?and","to":"explain—and"},{"from":"stress?such","to":"stress—such"},{"from":"topics?such","to":"topics—such"},{"from":"events?real","to":"events—real"},{"from":"growth?such","to":"growth—such"},{"from":"stories?and","to":"stories—and"},{"from":"mats?smooth","to":"mats—smooth"},{"from":"toy?showing","to":"toy—showing"},{"from":"visuals?for","to":"visuals—for"},{"from":"decade?like","to":"decade—like"},{"from":"hiccups?and","to":"hiccups—and"},{"from":"own?perhaps","to":"own—perhaps"},{"from":"routines?by","to":"routines—by"},{"from":"system?like","to":"system—like"},{"from":"chinese?and","to":"chinese—and"},{"from":"routes?like","to":"routes—like"},{"from":"lives?think","to":"lives—think"},{"from":"wall?filled","to":"wall—filled"},{"from":"layers?like","to":"layers—like"},{"from":"trends?like","to":"trends—like"},{"from":"matters?and","to":"matters—and"},{"from":"takes?ready","to":"takes—ready"},{"from":"trends?such","to":"trends—such"},{"from":"events?like","to":"events—like"},{"from":"states?calm","to":"states—calm"},{"from":"animal?like","to":"animal—like"},{"from":"scenes?such","to":"scenes—such"},{"from":"action?such","to":"action—such"},{"from":"apples?from","to":"apples—from"},{"from":"socks?using","to":"socks—using"},{"from":"mornings?by","to":"mornings—by"},{"from":"moments?how","to":"moments—how"},{"from":"source?like","to":"source—like"},{"from":"fuels?helps","to":"fuels—helps"},{"from":"group?peers","to":"group—peers"},{"from":"clearer?and","to":"clearer—and"},{"from":"imagine?and","to":"imagine—and"},{"from":"do?watching","to":"do—watching"},{"from":"medium?real","to":"medium—real"},{"from":"virtual?and","to":"virtual—and"},{"from":"else?unique","to":"else—unique"},{"from":"pieces?such","to":"pieces—such"},{"from":"travel?they","to":"travel—they"},{"from":"distort?the","to":"distort—the"},{"from":"sound?music","to":"sound—music"},{"from":"change?like","to":"change—like"},{"from":"picture?and","to":"picture—and"},{"from":"moon?making","to":"moon—making"},{"from":"sun?showing","to":"sun—showing"},{"from":"gadget?such","to":"gadget—such"},{"from":"pioneer?and","to":"pioneer—and"},{"from":"grow?family","to":"grow—family"},{"from":"audio?ready","to":"audio—ready"},{"from":"voices?real","to":"voices—real"},{"from":"voices?and","to":"voices—and"},{"from":"matter?and","to":"matter—and"},{"from":"voice?that","to":"voice—that"},{"from":"tales?then","to":"tales—then"},{"from":"built?like","to":"built—like"},{"from":"plant?such","to":"plant—such"},{"from":"plants?and","to":"plants—and"},{"from":"home?ready","to":"home—ready"},{"from":"new?photos","to":"new—photos"},{"from":"planet?one","to":"planet—one"},{"from":"crafted?to","to":"crafted—to"},{"from":"figure?and","to":"figure—and"},{"from":"desert?and","to":"desert—and"},{"from":"grades?can","to":"grades—can"},{"from":"system?and","to":"system—and"},{"from":"parents?to","to":"parents—to"},{"from":"shaped?and","to":"shaped—and"},{"from":"clues?such","to":"clues—such"},{"from":"decode?and","to":"decode—and"},{"from":"solve?real","to":"solve—real"},{"from":"mapped?and","to":"mapped—and"},{"from":"hats?using","to":"hats—using"},{"from":"stage?such","to":"stage—such"},{"from":"others?not","to":"others—not"},{"from":"artefact?a","to":"artefact—a"},{"from":"fabric?can","to":"fabric—can"},{"from":"waste?like","to":"waste—like"},{"from":"music?from","to":"music—from"},{"from":"lives?into","to":"lives—into"},{"from":"more?layer","to":"more—layer"},{"from":"nature?and","to":"nature—and"},{"from":"script?can","to":"script—can"},{"from":"sensors?to","to":"sensors—to"},{"from":"photo?each","to":"photo—each"},{"from":"stage?farm","to":"stage—farm"},{"from":"home?using","to":"home—using"},{"from":"spring?and","to":"spring—and"},{"from":"outside?it","to":"outside—it"},{"from":"rules?side","to":"rules—side"},{"from":"agency?and","to":"agency—and"},{"from":"plant?then","to":"plant—then"},{"from":"layers?tap","to":"layers—tap"},{"from":"life?every","to":"life—every"},{"from":"magic?they","to":"magic—they"},{"from":"arms?using","to":"arms—using"},{"from":"others?and","to":"others—and"},{"from":"debate?not","to":"debate—not"},{"from":"trade?like","to":"trade—like"},{"from":"goals?like","to":"goals—like"},{"from":"issue?like","to":"issue—like"},{"from":"gears?that","to":"gears—that"},{"from":"bark?shape","to":"bark—shape"},{"from":"animals?to","to":"animals—to"},{"from":"mural?made","to":"mural—made"},{"from":"petals?and","to":"petals—and"},{"from":"ideas?and","to":"ideas—and"},{"from":"story?and","to":"story—and"},{"from":"level?can","to":"level—can"},{"from":"bins?that","to":"bins—that"},{"from":"more?then","to":"more—then"},{"from":"loops?let","to":"loops—let"},{"from":"shape?the","to":"shape—the"},{"from":"life?both","to":"life—both"},{"from":"future?to","to":"future—to"},{"from":"alumni?to","to":"alumni—to"},{"from":"video?can","to":"video—can"},{"from":"toys?such","to":"toys—such"},{"from":"robot?and","to":"robot—and"},{"from":"works?one","to":"works—one"},{"from":"home?each","to":"home—each"},{"from":"future?by","to":"future—by"},{"from":"track?can","to":"track—can"},{"from":"caps?then","to":"caps—then"},{"from":"scraps?as","to":"scraps—as"},{"from":"item?such","to":"item—such"},{"from":"images?it","to":"images—it"},{"from":"butter?at","to":"butter—at"},{"from":"needs?and","to":"needs—and"},{"from":"world?and","to":"world—and"},{"from":"lines?and","to":"lines—and"},{"from":"cues?like","to":"cues—like"},{"from":"place?and","to":"place—and"},{"from":"peers?can","to":"peers—can"},{"from":"symbol?to","to":"symbol—to"},{"from":"joy?using","to":"joy—using"},{"from":"patch?and","to":"patch—and"},{"from":"life?such","to":"life—such"},{"from":"flowers?a","to":"flowers—a"},{"from":"path?even","to":"path—even"},{"from":"world?one","to":"world—one"},{"from":"pins?each","to":"pins—each"},{"from":"shape?and","to":"shape—and"},{"from":"videos?to","to":"videos—to"},{"from":"dramas?to","to":"dramas—to"},{"from":"matter?it","to":"matter—it"},{"from":"votes?can","to":"votes—can"},{"from":"lens?such","to":"lens—such"},{"from":"them?like","to":"them—like"},{"from":"shapes?to","to":"shapes—to"},{"from":"seeds?can","to":"seeds—can"},{"from":"make?like","to":"make—like"},{"from":"trees?and","to":"trees—and"},{"from":"idea?how","to":"idea—how"},{"from":"guard?to","to":"guard—to"},{"from":"time?and","to":"time—and"},{"from":"organ?to","to":"organ—to"},{"from":"spot?and","to":"spot—and"},{"from":"angle?on","to":"angle—on"},{"from":"notes?so","to":"notes—so"},{"from":"tools?by","to":"tools—by"},{"from":"told?and","to":"told—and"},{"from":"eye?able","to":"eye—able"},{"from":"book?and","to":"book—and"},{"from":"links?by","to":"links—by"},{"from":"rocks?by","to":"rocks—by"},{"from":"drawn?by","to":"drawn—by"},{"from":"text?are","to":"text—are"},{"from":"notes?to","to":"notes—to"},{"from":"art?then","to":"art—then"},{"from":"tough?to","to":"tough—to"},{"from":"future?a","to":"future—a"},{"from":"life?far","to":"life—far"},{"from":"job?such","to":"job—such"},{"from":"class?it","to":"class—it"},{"from":"downs?on","to":"downs—on"},{"from":"music?no","to":"music—no"},{"from":"say?even","to":"say—even"},{"from":"by?their","to":"by—their"},{"from":"them?and","to":"them—and"},{"from":"ideas?to","to":"ideas—to"},{"from":"shape?or","to":"shape—or"},{"from":"life?the","to":"life—the"},{"from":"out?then","to":"out—then"},{"from":"sun?like","to":"sun—like"},{"from":"home?by","to":"home—by"},{"from":"own?has","to":"own—has"},{"from":"data?no","to":"data—no"},{"from":"code?to","to":"code—to"},{"from":"sky?day","to":"sky—day"},{"from":"needs?a","to":"needs—a"},{"from":"file?an","to":"file—an"},{"from":"way?and","to":"way—and"},{"from":"kids?so","to":"kids—so"},{"from":"trek?on","to":"trek—on"},{"from":"own?and","to":"own—and"},{"from":"web?and","to":"web—and"},{"from":"toys?on","to":"toys—on"},{"from":"data?a","to":"data—a"},{"from":"us?by","to":"us—by"}],"space":[]};
var REPAIR_DASH_FIELDS = { d: true, from: true, fromTool: true };
var _REPAIR_COMPILED = null;

function repairCompiled_() {
  if (_REPAIR_COMPILED) return _REPAIR_COMPILED;
  function esc(s) { return s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); }
  function comp(list) {
    if (!list || !list.length) return null;
    var dict = {};
    var parts = list.map(function (m) { dict[m.from.toLowerCase()] = m.to; return esc(m.from); });
    return { re: new RegExp(parts.join('|'), 'gi'), dict: dict };
  }
  _REPAIR_COMPILED = { all: comp(REPAIR_TEXT_MAP.all), dash: comp(REPAIR_TEXT_MAP.dash), space: comp(REPAIR_TEXT_MAP.space) };
  return _REPAIR_COMPILED;
}

function repairApplyCase_(hit, to) {
  if (hit === hit.toUpperCase() && /[A-Z]/.test(hit)) return to.toUpperCase();
  if (hit.length === to.length) {
    var out = '';
    for (var i = 0; i < to.length; i++) {
      var h = hit.charAt(i), t = to.charAt(i);
      out += (h === h.toUpperCase() && h !== h.toLowerCase()) ? t.toUpperCase() : t;
    }
    return out;
  }
  return (hit.charAt(0) === hit.charAt(0).toUpperCase() && hit.charAt(0) !== hit.charAt(0).toLowerCase())
...
function repairScrambledText(opts) {
  ...
  function applyPack(s, pack) {
    if (!pack) return s;
    pack.re.lastIndex = 0;
    return s.replace(pack.re, function (hit) {
      var to = pack.dict[hit.toLowerCase()];
      if (to === undefined) return hit;
      stats.total++;
      var k = hit.toLowerCase();
      perToken[k] = (perToken[k] || 0) + 1;
      return repairApplyCase_(hit, to);
    });
  }
  function fixString(s, key) {
    return applyPack(applyPack(s, maps.all), REPAIR_DASH_FIELDS[key] ? maps.dash : maps.space);
  }
  var unitsTouched = 0;
  function walk(o, key) {
    var changed = false;
    if (Array.isArray(o)) {
      for (var i = 0; i < o.length; i++) {
        var v = o[i];
        if (typeof v === 'string') { var f = fixString(v, key); if (f !== v) { o[i] = f; changed = true; } }
        else if (v && typeof v === 'object') { if (walk(v, key)) changed = true; }
      }
    } else {
      for (var k in o) {
        if (!Object.prototype.hasOwnProperty.call(o, k)) continue;
        var v2 = o[k];
        if (typeof v2 === 'string') { var f2 = fixString(v2, k); if (f2 !== v2) { o[k] = f2; changed = true; } }
        else if (v2 && typeof v2 === 'object') { if (walk(v2, k)) changed = true; }
      }
    }
    return changed;
  }
